...
export const SecurityDashboardPage: React.FC = () => {
    ...
    const refreshMetrics = () => {
        ...
        // In prod, rules would have 'technique_id' tags. Here we infer or randomize for demo.
        ...
        rules.forEach(r => {
            if (r.name.includes('Brute')) coverage['Brute Force'] = (coverage['Brute Force'] || 0) + 1;
            if (r.name.includes('Root') || r.name.includes('Sudo')) coverage['Sudo'] = (coverage['Sudo'] || 0) + 1;
            // Add some synthetic coverage for the heatmap visual
        });

        // Synthetic data to populate heatmap
        setMitreCoverage({
        ...
        const handleExplainRisk = async () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    // Fallback for demo if API fails or backend not running locally in same context
                    setExplanation({
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <Card className="p-0 border-slate-700 bg-slate-800/40 h-80 flex flex-col">
                                ...
                                <div className="flex-1 flex items-center justify-center text-slate-500 italic">
                                    {/* Construct a mock bar chart using flex divs */}
